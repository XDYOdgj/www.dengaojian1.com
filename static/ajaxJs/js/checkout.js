...
const shopCart = () => {
	...
	storage.getStorageData("cart").forEach(item => {
		html = html + `	<tr class="cart_item" >
						  <td class="product-name" style="width:80%">${item.goods.name}&nbsp;&nbsp; <strong
							 class="product-quantity">×${item.num}</strong></td>
							<td class="product-total"  style="width:20%" >
							<span class="lynessa-Price-amount amount"><span
								class="lynessa-Price-currencySymbol">$</span>${calculation.preciseMultiply(Number(item.price), item.num)}</span>
							</td>
						</tr> `
